...
function markTaskDone(weekStart, taskId, staffName, done = true) {
  ...
  // Push tick or untick to Sheets so other devices stay in sync
  const record = {
    id:        'task_' + Date.now(),
    ...
    fields: {
      task_id:      taskId,
      task_week:    weekStart,
      task_done_by: done ? staffName : '',
      task_action:  done ? 'done' : 'untick',
    },
    summary: done ? `Task completed by ${staffName}` : `Task unticked`,
  };
