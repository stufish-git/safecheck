...
function doPost(e) {
  try {
    ...
    if (!sheet) {
      ...
      // Sheet exists — check if headers need extending (handles schema additions)
      const existingHeaders = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      if (existingHeaders.length < data.headers.length) {
        // Add missing header columns without touching existing data
        const startCol = existingHeaders.length + 1;
        const newCols  = data.headers.slice(existingHeaders.length);
        const range    = sheet.getRange(1, startCol, 1, newCols.length);
